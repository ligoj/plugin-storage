/*
 * Service layer for plugin "storage" (Storage, service-level).
 *
 * The legacy `service/storage/storage.js` was `define({})` (or a thin base
 ...
 * `renderDetailsFeatures`) to the storage-<tool> sub-plugin resolved from
...
/**
 * Derive the sub-plugin id for a storage tool subscription. A storage node id
 * is `service:storage:<tool>[:<instance>]` — segment 3 is the tool, so
 * `service:storage:<tool>:1` → `storage-<tool>`. Returns null when there is no
 * tool segment to delegate to.
 */
export const subPluginIdFor = toolPluginId

/** Delegate `action` to the storage-<tool> sub-plugin; `[]` on any failure. */
export const delegateToToolPlugin = (subscription, action) => delegateFeature(subscription, action, 'storage')

const service = {
  subPluginIdFor,
  delegateToToolPlugin,

  /** Subscription-row buttons — delegated wholesale to the storage-<tool>. */
